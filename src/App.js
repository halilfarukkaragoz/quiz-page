  ...
  displayQuiz = wiewHeader => {
    ...
    if (questions[0] != undefined) {
      this.setState({
        wiewHeader: false,
        wiewQuestions: true,
        displayQuestion: Object.assign(questions[0])
      });
    }
    
    if (questions[0] != undefined) this.settingAnswers(0
      );
  };
  settingAnswers(a) {
  ...
  render() {
    ...
    return (
      <div>
        {wiewHeader && (
          <Header
            wiewHeader={wiewHeader}
            displayQuiz={this.displayQuiz}
            questions={questions}
          />
        )}
        {wiewQuestions && (
          <Question
            answers={answers}
            displayScorebox={this.displayScorebox}
            setScore={this.setScore}
            score={score}
            numberOfQuestions={this.state.numberOfQuestions}
            displayQuestion={displayQuestion}
            questions={questions}
            changingQuestion={this.changingQuestion}
          />
        )}
        {wiewScorebox && <Scorebox score={score} />}
      </div>
    );
  }
}
